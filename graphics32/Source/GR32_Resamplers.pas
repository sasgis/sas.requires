...
procedure BlockTransferExt(
  ...
  SrcCombineMode: TCombineMode = cmMerge;
  SrcMasterAlpha: Cardinal = 255;
  SrcOuterColor: TColor32 = 0;
...
procedure BlendBlock(
  ...
  SrcX, SrcY: Integer;
  SrcOuterColor: TColor32;
  CombineOp: TDrawMode;
  SrcCombineMode: TCombineMode;
  SrcMasterAlpha: Cardinal;
...
begin
  ...
  case CombineOp of
    ...
    dmBlend:
      if SrcMasterAlpha >= 255 then
      begin
        BlendLine := BLEND_LINE[SrcCombineMode]^;
        for DstY := DstRect.Top to DstRect.Bottom - 1 do
        begin
      ...
      begin
        BlendLineEx := BLEND_LINE_EX[SrcCombineMode]^;
        for DstY := DstRect.Top to DstRect.Bottom - 1 do
        begin
    ...
    dmTransparent:
      ...
      begin
        ...
        begin
          ...
          for I := 0 to W - 1 do
          begin
            CombineCallBack(SP^, DP^, SrcMasterAlpha);
            Inc(SP); Inc(DP);
          end;
          Inc(SrcP, SrcWidth);
          Inc(DstP, Dst.Width);
...
begin
  ...
  if Dst.Empty or not Assigned(SrcBits) or (SrcWidth <= 0) or (SrcHeight <= 0) or
    ((CombineOp = dmBlend) and (SrcMasterAlpha = 0)) then Exit;
  ...
  begin
    ...
    BlendBlock(Dst, DstClip, SrcBits, SrcWidth, SrcHeight, SrcRect.Left, SrcRect.Top, SrcOuterColor, CombineOp, SrcCombineMode, SrcMasterAlpha, CombineCallBack);
  end;
...
begin
  ...
  begin
    ...
    BlendBlock(Dst, DstClip, SrcBits, SrcWidth, SrcHeight,
      ...
      SrcRect.Top + DstClip.Top - DstRect.Top, SrcOuterColor, CombineOp, SrcCombineMode,
      SrcMasterAlpha, CombineCallBack);
  end
  else
  begin
          ...
          case CombineOp of
            dmBlend:
              if SrcMasterAlpha >= 255 then
                ...
                BlendLineEx(@Buffer[0], @DstLine[0], DstClipW, SrcMasterAlpha);
            dmTransparent:
              for I := 0 to DstClipW - 1 do
                if Buffer[I] <> SrcOuterColor then DstLine[I] := Buffer[I];
            dmCustom:
              for I := 0 to DstClipW - 1 do
                CombineCallBack(Buffer[I], DstLine[I], SrcMasterAlpha);
          end;
...
{$ifend}
    ...
    dmBlend:
      begin
        BlendMemEx := BLEND_MEM_EX[SrcCombineMode]^;
        for J := 0 to DstClipH - 1 do
        begin
          SrcLine := @SrcBits[MapVert[J].Pos * SrcWidth];
          WY := MapVert[J].Weight;
...
{$else}
        C := AlphaInterpolator(MapHorz[I].Weight, WY, SrcPtr1, SrcPtr2);
{$ifend}
        CombineCallBack(C, DstLine[I], SrcMasterAlpha);
      end;
      Inc(DstLine, Dst.Width);
    end;
  end;
...
begin
  ...
  if (CombineOp = dmBlend) and (SrcMasterAlpha = 0) then
    Exit;

  BlendMemEx := BLEND_MEM_EX[SrcCombineMode]^; // store in local variable
...
{$endif PREMULTIPLY}
      ...
      case CombineOp of
        ...
        dmBlend:
          BlendMemEx(C.ARGB, DstLine[I], SrcMasterAlpha);

        dmTransparent:
          if C.ARGB <> SrcOuterColor then
            DstLine[I] := C.ARGB;

        dmCustom:
          CombineCallBack(C.ARGB, DstLine[I], SrcMasterAlpha);
      end;
    end;
  end;
end;
...
procedure DraftResample(Dst: TCustomBitmap32; DstRect: TRect; DstClip: TRect;
  SrcBits: PColor32Array; SrcWidth, SrcHeight: Integer; SrcRect: TRect; Kernel: TCustomKernel;
  SrcOuterColor: TColor32;
  CombineOp: TDrawMode;
  SrcCombineMode: TCombineMode;
  SrcMasterAlpha: Cardinal;
 ...
 { rangechecking and rect intersection done by caller }
  ...
  begin
    if (SrcW < 2) or (SrcH < 2) then
      Resample(Dst, DstRect, DstClip, SrcBits, SrcWidth, SrcHeight, SrcRect, Kernel, SrcOuterColor, CombineOp, SrcCombineMode, SrcMasterAlpha, CombineCallBack)
    else
      StretchHorzStretchVertLinear(Dst, DstRect, DstClip, SrcBits, SrcWidth, SrcHeight, SrcRect, SrcOuterColor, CombineOp, SrcCombineMode, SrcMasterAlpha, CombineCallBack);
  end else
        ...
        dmBlend:
          ...
          begin
            ...
            BlendMemEx(BlockAverage(dx, dy, xsrc, OffSrc),
              DstLine[DstClip.Left + I], SrcMasterAlpha);
            Inc(xsrc, dx);
          end;
...
begin
  ...
  if not Assigned(SrcBits) or (SrcWidth <= 0) or (SrcHeight <= 0) or Dst.Empty or
    ((CombineOp = dmBlend) and (SrcMasterAlpha = 0)) or
  ...
  begin
    ...
    if (SrcW = DstW) and (SrcH = DstH) then
      BlendBlock(Dst, DstClip, SrcBits, SrcWidth, SrcHeight, SrcRect.Left + DstClip.Left - DstRect.Left, SrcRect.Top + DstClip.Top - DstRect.Top, SrcOuterColor, CombineOp, SrcCombineMode, SrcMasterAlpha, CombineCallBack)
    else
      TCustomResamplerAccess(Resampler).Resample(Dst, DstRect, DstClip, SrcBits, SrcWidth, SrcHeight, SrcRect, CombineOp, SrcCombineMode, SrcMasterAlpha, SrcOuterColor, CombineCallBack);
  end;
